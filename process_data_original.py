...
def process_data_for_target(target, combined_data):
    ...
    X = impute_missing_values_spline(X, column)
    y = impute_missing_values_spline(y, target)
    ...
    # Apply Percentage Change (after splitting to avoid data leakage) -- WRONG
    # Removed this step, as pct_change() is now applied before splitting, no data leakage occurs
